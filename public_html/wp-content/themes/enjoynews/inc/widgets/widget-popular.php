 ...
 * @since      1.0.0
 */
class EnjoyNews_Most_Commented_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-enjoynews-most-commented widget-posts-thumbnail',
			'description' => __( 'Display most commented posts with thumbnails.', 'enjoynews' )
		);

		// Create the widget.
		parent::__construct(
			'enjoynews-most-commented', // $this->id_base
			__( '&raquo; Most Commented Posts', 'enjoynews' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		if ( false === ( $popular = get_transient( 'enjoynews_most_commented_widget_' . $this->id ) ) ) {
			...
			// Store the transient.
			set_transient( 'enjoynews_most_commented_widget_' . $this->id, $popular );
	...
	function flush_widget_transient() {
		delete_transient( 'enjoynews_most_commented_widget_' . $this->id );
	}
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'     => esc_html__( 'Most Commented Posts', 'enjoynews' ),
			'limit'     => 5,
			'show_date' => true
		);
